...
export const Profile = () => {
    ...
    const handleSaveProfile = async (e: FormEvent) => {
        ...
        if (!displayName.trim()) {
            errorfulToast('❌ Display name cannot be empty.');
            return;
        }
        ...
        if (updateProfile.fulfilled.match(result)) {
            successfulToast('✅ Profile updated successfully.');
            setIsEditing(false);
        } else {
            errorfulToast(error || '❌ Failed to update profile.');
        }
    };
    ...
    const handleDeleteAccount = async () => {
        if (deleteConfirmText !== 'DELETE') {
            errorfulToast('❌ Please type DELETE to confirm.');
            return;
        }
        ...
        try {
            ...
            if (!currentUser) {
                errorfulToast('❌ No authenticated user.');
                return;
            }
            ...
            successfulToast('✅ Account deleted successfully.');
            navigate('/register');
        } catch (error: unknown) {
            if ((error as { code?: string }).code === 'auth/requires-recent-login') {
                errorfulToast('❌ Please log out and log back in before deleting your account.');
            } else {
                errorfulToast('❌ Failed to delete account.');
            }
                            ...
                            <div className='relative'>
                                {user.photoURL ? (
                                    <img
                                        ...
                                        className='w-20 h-20 rounded-full border-2 border-accent shadow-cyan object-cover'
                                    />
                                ) : (
